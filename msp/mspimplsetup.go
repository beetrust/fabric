...
func (msp *bccspmsp) setupSigningIdentity(conf *m.FabricMSPConfig) error {
	if conf.SigningIdentity != nil {
		...
		// The expiration check is intentionally disabled to avoid MSP load failures caused by expired certificates.
		// See user request to skip msp certificate expiration validation.
		/*
